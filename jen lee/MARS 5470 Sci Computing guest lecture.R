...
fert <- read.csv("C:/Users/Chris/Google Drive/000 R-stats/shared/Sci Comp guest lecture/basic_stats.csv", header=T)
...
## it is the error or "wiggle" in the best fir line
## EXERCISE: How is bowtie tilt related to statistical significance of a regression model?
##if the slope is zero there is no significance
## Correlation tests
...
## The slope of the line can be positive or negative and this determines the relationship between variables. 
## The linear model can also have a slope of noneor zero this shows no relationship. 
## linear models give you p-values which if they are higher than 0.05 the relationship is significcant.

### 3.1.2 - Was there a relationship between soil moisture and change in height?

# visualize
plot(fert$soil.moisture, fert$change.height)
tggplot(fert, aes(soil.moisture, change.height)) + geom_point() 
...
## For this line there is a positive relationship, nowtie tilt is the room or error for the best fir line.
# correlation test
...
## Yes, height increases as soil moisture increases.
...
b## EXERCISE: How does one visualize a multiple regression?
## The best fit line determines importance 
# linear model
...
## N0o not normal since there is not a bell shape curve and there is heavy tails.
...
## p-value = 3.932 is very low, rejectinng the null hypothesis which means its not normal.
## Fit the model. 
...
# It is a false positive. the answer is Contrary to popular belief, data do NOT need to be normal to meet the assumptions of ANOVA.
...
# Yes, it is higher than the control
## visualize
...
## There is no significance or relationship, line slope is almost zero.
...
# Climate change changing plant community in diffrent wetlands around the gulf coast.
...
# Model showed no significance.Bioflof persumably did not work, Brine shrimp experienced high mortality.
